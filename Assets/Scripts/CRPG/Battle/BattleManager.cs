...
public static class BattleManager
{
    ...
    [System.NonSerialized] public static UnityEvent<Personage> OnPersonageJoinBattleEvent = new();
    ...
    public static void JoinToBattle(Personage personage)
    {
        personage.OnDeath.AddListener(() => OnPersonageDeath(personage));
        Personage activePersonage = ActivePersonage;
        AddToInitiative(personage);
        _activePersonageIndex = _participantPersonages.IndexOf(activePersonage);
        OnPersonageJoinBattleEvent.Invoke(personage);
    }
    ...
    private static void AddToInitiative(Personage personage)
    {
        int initiative = CharacteristicChecker.RoleCharacteristic(personage.PersonageInfo, Characteristics.Dexterity);
        _personagesInitiative[personage] = initiative;
        _participantPersonages = (from pers in _personagesInitiative orderby pers.Value descending select pers.Key).ToList();

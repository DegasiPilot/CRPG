...
public class GameManager : MonoBehaviour
{
    ...
    private void ShowAttackInfo(Personage personage)
    {
        ...
        attackInfoBuider.AppendLine($"Расстояние до цели {string.Format("{0:F1}",distance)}");
    ...
    public void AttackInteract(Component component)
    {
        ...
        IEnumerator AttackInteract()
        {
            ...
            if (_gameMode != GameMode.Battle)
            {
                ...
                BattleManager.StartBattle(new Personage[2] { PlayerPersonage, personageController.Personage });
            }
            else if (!BattleManager.ParticipantPersonages.Contains(personageController.Personage)){
                BattleManager.JoinToBattle(personageController.Personage);
            }
        }
    }
